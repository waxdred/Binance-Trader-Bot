    ...
    loop {
        ...
        if !trade.data.other_position_ret_list.is_empty() {
            ...
            if tmp.len() > history.len() {
                for t in tmp.iter() {
                    ...
                    if check {
                        ...
                        } else if !configs.blacklist.is_empty()
                            && configs.blacklist.contains(&t.symbol.clone())
                        {
                            continue;
                        } else {
                            history.insert(0, t.clone());
                        ...
                        sleep(Duration::from_secs(configs.delai.clone())).await;
                        //add time sleep
                    }
                }
            }
            if !history.is_empty() {
                ...
                history.retain(|x| {
                    !tmp.iter()
                        .any(|y| y.update_time_stamp == x.update_time_stamp && y.symbol == x.symbol)
                });
